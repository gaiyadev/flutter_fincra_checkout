...
void main() {
  group('UrlHandler', () {
    test('isCompletionUrl returns true when status and reference are present', () {
      final url = 'https://myapp.com/callback?status=success&reference=REF123';
    ...
    test('isCompletionUrl checks expectedRedirectUrl if provided', () {
      ...
      expect(UrlHandler.isCompletionUrl(url, expectedRedirectUrl: 'https://myapp.com/callback'), isTrue);
      expect(UrlHandler.isCompletionUrl(url, expectedRedirectUrl: 'https://otherapp.com/callback'), isFalse);
    });

    test('extractResponseParams extracts correctly', () {
      final url = 'https://myapp.com/callback?status=failed&reference=REF123&message=Insufficient+funds';
